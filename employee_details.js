...
function displayEmployees() {
    const totalEmployees = employees
        .map(employee => <p>${employees.id}: ${employee.name} - ${employee.department} - $${employee.salary}</p>)
        .join('');
...
function displayHREmployees() {
    ...
    document.getElementById('employeesDetails').innerHTML = hrEmployees.display;
}
...
function findEmployeeById(employeeId){
    ...
    if (foundEmployee){
        document.getElementById('employeeDetails').innerHTML = `<p>${foundEmployee.id}: ${foundEmployee.name}: ${foundEmployee.name} - ${foundEmployee.department} - $${foundEmployee.salary}</p>`.join('');
    }
    else{
